var  ws_url = 'ws://192.168.2.100:9090';
...
const MAX_DOWN = 0.01
const MAX_UP = 0.01
const MAX_ANGULAR = 0.00025
const MAX_PITCH_ANGULAR = 0.01
let thr_config_name
let thr_config_position
let mission = []

let G_MAX_FORWARD_BACKWARD = [-0.0015, -0.0035, -0.007] // -1 
const G_MAX_BACKWARD = 1
let G_MAX_UP_DOWN = [-0.4, -0.6, -1.0] // -1 
const G_MAX_UP = 1
let G_MAX_ANGULAR = [0.00025, 0.0003, 0.0005] // 0.15 
let G_MAX_PITCH_ANGULAR = [0.1, 0.1, 0.1] //0.2

let G_K_id = 0 
let G_K_id_led = 0 
let G_K_color = ['green', 'red', 'yellow']
let G_K_color_speed = ['green', 'yellow', 'red']

function G_mode(){
    if (G_MAX_FORWARD_BACKWARD.length -2 < G_K_id_led){
        G_K_id_led = 0;
    } else {
        G_K_id_led++;
    }
    console.log("LED:");
    console.log(G_K_id_led);
    document.getElementById('mode').style.backgroundColor = G_K_color[G_K_id_led]
    var cmd = new ROSLIB.Message({
        data: G_K_id_led + 1
    });

    light_signal_publisher.publish(cmd)
}

function G_speed_mode(){
    ...
    console.log("speed:");
    console.log(G_K_id);
    document.getElementById('speed_mode').style.backgroundColor = G_K_color_speed[G_K_id]
}

document.getElementById('mode').onclick = function(){
    G_mode();
}
document.getElementById('speed_mode').onclick = function(){
    G_speed_mode();
}
...
function control() {
    if (control_type == 'web'){
        forward_backward = joy_right.GetY()*-1
    ...
    if (control_type == 'web_joy'){
        ...
        var cmd = new ROSLIB.Message({        
            // force: {
            linear: {
                x: forward_backward > 0 ? forward_backward * 100 * G_MAX_FORWARD_BACKWARD[G_K_id] : forward_backward * 100 * G_MAX_FORWARD_BACKWARD[G_K_id] ,
            ...
            angular: {
                ...
                z: (left_right * G_MAX_ANGULAR[G_K_id] * 100 )
            }
        });
        cmd_publisher.publish(cmd)                
...
// G_mode();
            ...
            if (gamepadAPI.buttonPressed("X", 0)){
                G_speed_mode();
            }
        }
    }
